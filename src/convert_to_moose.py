...
def write_moose_snippet(
    ...
    """
    Generate a ready-to-use MOOSE .i snippet with:
      [Mesh]            — FileMeshGenerator + 6 ParsedGenerateSideset + BreakMeshByBlockGenerator
      [InterfaceKernels] — one HomogenizedExponentialCZM per interface type
      [BCs]              — 6 external face boundary conditions (stubs)
    """
    b = bounds
    # Tolerance strings — slightly inside the face
    ...
    # Collect unique interface types
    ...
    # Group interfaces by type for InterfaceKernels
    ...
    lines.append("# Paste into your simulation .i file and adjust as needed.")
    lines.append("# ============================================================")
    lines.append("")
    ...
    lines.append("    use_for_exodus_restart = true")
    lines.append("  []")
    lines.append("")
    lines.append("  # ── External face sidesets (adjust tolerances if needed) ──")
    ...
    for name, expr, inp in sideset_order:
        lines.append(f"  [{name}]")
    ...
    lines.append("")
    lines.append("  # ── Break mesh at block boundaries to create interface sidesets ──")
    lines.append("  [break]")
    ...
    # ── [InterfaceKernels] ────────────────────────────────────────────────────
    lines.append("[InterfaceKernels]")

    if has_intra and intra_pairs:
        lines.append("  # ── Intra-grain interfaces (needle-needle, same sclerodermite) ──")
        lines.append("  # CZM parameters: organic matrix / tightly bonded")
        for bA, bB in intra_pairs:
            iface_name = f"interface_{bA}_{bB}"
            lines.append(f"  [{iface_name}]")
            lines.append(f"    type = HomogenizedExponentialCZM")
            lines.append(f"    boundary = 'interface_block{bA}_block{bB}'")
            lines.append(f"    base_name = '{czm_intra}'")
            lines.append(f"    displacements = 'disp_x disp_y disp_z'")
            lines.append(f"    component = 0")
            lines.append(f"  []")

    if has_inter and inter_pairs:
        lines.append("")
        lines.append("  # ── Inter-grain interfaces (across sclerodermite boundaries) ──")
        lines.append("  # CZM parameters: protein / water layer")
        for bA, bB in inter_pairs:
            iface_name = f"interface_{bA}_{bB}"
            lines.append(f"  [{iface_name}]")
            lines.append(f"    type = HomogenizedExponentialCZM")
            lines.append(f"    boundary = 'interface_block{bA}_block{bB}'")
            lines.append(f"    base_name = '{czm_inter}'")
            lines.append(f"    displacements = 'disp_x disp_y disp_z'")
            lines.append(f"    component = 0")
            lines.append(f"  []")
    ...
    # ── [BCs] ─────────────────────────────────────────────────────────────────
    lines.append("[BCs]")
    lines.append("  # ── External boundaries — adjust type and values for your load case ──")
    lines.append("  [fix_bottom_x]")
    ...
    lines.append("    value = 0")
    lines.append("  []")
    lines.append("  [fix_bottom_z]")
    lines.append("    type = DirichletBC")
    lines.append("    boundary = bottom")
    lines.append("    variable = disp_z")
    lines.append("    value = 0")
    lines.append("  []")
    lines.append("  [load_top_z]")
    lines.append("    type = FunctionDirichletBC")
    lines.append("    boundary = top")
    lines.append("    variable = disp_z")
    lines.append(f"    function = 't * {b['zmax'] - b['zmin']:.4f} * 0.03'  # 3% strain")
    lines.append("  []")
    lines.append("[]")
    lines.append("")
    ...
    lines.append(f"# Mesh bounds:  X [{b['xmin']:.4f}, {b['xmax']:.4f}]")
    ...
    lines.append(f"#               {len(intra_pairs)} intra-grain  → material '{czm_intra}'")
    lines.append(f"#               {len(inter_pairs)} inter-grain  → material '{czm_inter}'")
    lines.append(f"# CZM material blocks needed in [Materials]:")
    if has_intra:
        lines.append(f"#   base_name = '{czm_intra}'  (intra-grain CZM parameters)")
    if has_inter:
        lines.append(f"#   base_name = '{czm_inter}'  (inter-grain CZM parameters)")
